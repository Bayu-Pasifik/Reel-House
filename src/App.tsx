...
function DetailPage({ movieId, onBack, onSelect, onTrailer }: { movieId: number; onBack: () => void; onSelect: (movie: Movie) => void; onTrailer: (movie: Movie) => void }) {
  const details = useQuery({ queryKey: ['movie', movieId], queryFn: () => getMovieDetails(movieId) })
  const recommendations = useQuery({ queryKey: ['movie', movieId, 'recommendations'], queryFn: () => getRecommendations(movieId) })
  const credits = useQuery({ queryKey: ['movie', movieId, 'credits'], queryFn: () => getMovieCredits(movieId) })
  const watchProviders = useQuery({ queryKey: ['movie', movieId, 'watch-providers', 'ID'], queryFn: () => getWatchProviders(movieId) })
  const reviews = useQuery({ queryKey: ['movie', movieId, 'reviews'], queryFn: () => getMovieReviews(movieId) })
  const gallery = useQuery({ queryKey: ['movie', movieId, 'images'], queryFn: () => getMovieImages(movieId) })
  const collection = useQuery({ queryKey: ['collection', details.data?.belongs_to_collection?.id], queryFn: () => getCollection(details.data!.belongs_to_collection!.id), enabled: Boolean(details.data?.belongs_to_collection) })
  ...
  return <main className="detail-page">
    ...
    {details.isLoading ? <p className="detail-status status">Opening the film file…</p> : details.data ? <><section className="detail-hero" style={details.data.backdrop_path ? { backgroundImage: `url(${image(details.data.backdrop_path, 'original')})` } : undefined}><div className="detail-hero-shade" /><div className="detail-hero-copy"><p>{details.data.genres.map(genre => genre.name).join(' / ') || 'Feature film'} · {year(details.data.release_date)}</p><h1>{details.data.title}</h1>{details.data.tagline && <blockquote>{details.data.tagline}</blockquote>}<div className="detail-facts"><span>{details.data.vote_average.toFixed(1)} rating</span>{details.data.runtime && <span>{details.data.runtime} min</span>}<span>{details.data.status}</span><button className="watch-trailer" onClick={() => onTrailer(details.data)}>Watch trailer <i>▶</i></button></div></div></section><section className="detail-content"><div className="detail-summary"><h2>About the film</h2><p>{details.data.overview || 'No synopsis is available for this title.'}</p></div>{collection.data?.parts.length ? <section className="collection" aria-label={`The ${collection.data.name} collection`}><h2>{collection.data.name}</h2>{collection.data.overview && <p>{collection.data.overview}</p>}<div>{collection.data.parts.slice(0, 8).map(item => <Poster key={item.id} movie={item} onSelect={onSelect} />)}</div></section> : null}{gallery.data?.backdrops.length ? <section className="gallery" aria-label="Film gallery"><h2>Visual world</h2><div>{gallery.data.backdrops.sort((a, b) => b.vote_average - a.vote_average).slice(0, 4).map(backdrop => <img key={backdrop.file_path} src={image(backdrop.file_path, 'w1280')} alt={`Scene from ${details.data.title}`} loading="lazy" />)}</div></section> : null}{availability && <section className="watch-providers" aria-label="Where to watch in Indonesia"><div><h2>Where to watch</h2><p>Available in Indonesia</p></div><div className="provider-groups">{([['Stream', availability.flatrate], ['Rent', availability.rent], ['Buy', availability.buy]] as const).filter(([, providers]) => providers?.length).map(([label, providers]) => <div className="provider-group" key={label}><span>{label}</span><div>{providers!.slice(0, 6).map(provider => <a href={availability.link} target="_blank" rel="noreferrer" key={provider.provider_id} title={provider.provider_name}>{provider.logo_path ? <img src={image(provider.logo_path, 'w185')} alt={provider.provider_name} /> : <b>{provider.provider_name}</b>}</a>)}</div></div>)}</div></section>}{credits.data?.cast.length ? <section className="credits" aria-label="Cast and crew"><h2>{director ? `Directed by ${director.name}` : 'Featuring'}</h2><div className="cast-track">{credits.data.cast.slice(0, 6).map(person => <article className="cast-member" key={person.id}>{person.profile_path ? <img src={image(person.profile_path, 'w185')} alt="" loading="lazy" /> : <div className="cast-placeholder" />}<p>{person.name}</p><span>{person.character}</span></article>)}</div></section> : null}{reviews.data?.results.length ? <section className="reviews" aria-label="Audience reviews"><h2>Audience notes</h2><div>{reviews.data.results.slice(0, 3).map(review => <article key={review.id}><header><strong>{review.author}</strong>{review.author_details.rating !== null && <span>{review.author_details.rating.toFixed(1)} / 10</span>}</header><p>{review.content}</p><small>{new Date(review.created_at).toLocaleDateString('en', { year: 'numeric', month: 'short', day: 'numeric' })}</small></article>)}</div></section> : null}{recommendations.data?.results.length ? <section className="recommendations" aria-label="Recommended films"><h2>More to discover</h2><div className="recommendation-track">{recommendations.data.results.slice(0, 6).map(item => <Poster key={item.id} movie={item} onSelect={onSelect} />)}</div></section> : null}</section></> : <p className="detail-status status">Film details are unavailable. Return to discovery and try another title.</p>}
  </main>
}
